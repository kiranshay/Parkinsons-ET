...
def main():
    ...
    data_path = "/Users/kiranshay/projects/Parkinsons-ET/pads-dataset"  # Path to your dataset folder
    ...
    # Optional: Extract patient metadata features
    def extract_patient_metadata_features(patient_row):
        ...
        for col in numerical_cols:
            if col in patient_row and pd.notna(patient_row[col]):
                ...
                metadata_features.append(0.0)  # Default value for missing data
        
        # Categorical features (one-hot encoded)
        # Gender
        metadata_features.append(1.0 if patient_row.get('gender') == 'male' else 0.0)
        metadata_features.append(1.0 if patient_row.get('gender') == 'female' else 0.0)
        
        # Handedness
        metadata_features.append(1.0 if patient_row.get('handedness') == 'right' else 0.0)
        metadata_features.append(1.0 if patient_row.get('handedness') == 'left' else 0.0)
    ...
    print(f"\nExtracting features from {len(patients_df)} patients...")
    ...
    for idx, patient in patients_df.iterrows():
        ...
        # Add patient metadata features (optional)
        metadata_features = extract_patient_metadata_features(patient)
        patient_features.extend(metadata_features)
